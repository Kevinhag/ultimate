...
for pn in ivalues(GAMESTATE:GetHumanPlayers()) do 
	...
	local panel = Def.ActorFrame{
		InitCommand=function (self)
			self:x(SCREEN_CENTER_X + (140*pnSide(pn))):y(SCREEN_CENTER_Y-148);
		end;

		-- bg
		Def.Quad{
			InitCommand=function (self)
				self:vertalign(top):zoomto(350*pnSide(pn),210):diffuse(0,0,0,0.25):diffusebottomedge(0,0,0,0):faderight(0.25):fadeleft(0.25);
			end;
		},
		...
		Def.Sprite{
			InitCommand=function (self)
				self:vertalign(top):x(-86 - 16):y(12):zoom(0.15):diffusealpha(0.2);
			end;
		...
		Def.Sprite{
			InitCommand=function (self)
				self:vertalign(top):x(-86):y(12):zoom(0.3);
			end;
			UpdateScoresMessageCommand=function(self)
				local grade = nil;
		...
		Def.BitmapText{
			...
			InitCommand=function (self)
				self:zoom(0.38):diffuse(PlayerColor(pn)):strokecolor(BoostColor(PlayerColor(pn),0.3)):vertalign(top):x(-86):y(74):wrapwidthpixels(200):maxwidth(200):vertspacing(-6);
			end;
			UpdateScoresMessageCommand=function(self)
				if personal_score[pn] then
		...
		Def.BitmapText{
			...
			Text = "98.76%";
			InitCommand = function (self)
				self:zoom(0.475):vertalign(top):x(-86):y(90+13):strokecolor(0.2,0.2,0.2,1);
			end;
		...
		Def.BitmapText{
			...
			Text = "99.87%";
			InitCommand = function (self)
				self:zoom(0.45):vertalign(top):x(-86):y(106+13):diffuse(2/3,2/3,2/3,0.5):strokecolor(0.2,0.2,0.2,1);
			end;
	...
	for i=1,#hs_grade do 
		...
		panel[#panel+1] = Def.BitmapText{
			...
			InitCommand = function (self)
				self:zoomx(label_size):zoomy(label_size*0.95):horizalign(right):vertalign(top):vertspacing(-6):x(8):y(label_pos + ((i-1)*label_spacing));
			end;

			OnCommand = function (self)
				self:diffuse(hs_grade[i].Color):strokecolor(BoostColor(hs_grade[i].Color,0.3));
			end;
		...
		panel[#panel+1] = Def.BitmapText{
			...
			InitCommand=function (self)
				self:zoomx(label_size):zoomy(label_size*0.95):horizalign(left):vertalign(top):vertspacing(-6):x(36):y(label_pos + ((i-1)*label_spacing));
			end;
		...
		panel[#panel+1] = Def.BitmapText{
			Font = Fonts.highscores["Main"];
			InitCommand=function (self)
				self:zoomx(label_size):zoomy(label_size*0.95):horizalign(left):vertalign(top):vertspacing(-6):x(84):y(label_pos + ((i-1)*label_spacing));
			end;

			OnCommand=function (self)
				self:diffuse(2/3,2/3,2/3,1):strokecolor(0.25,0.25,0.25,1);
			end;
	...
	t[#t+1] = Def.ActorFrame{
        ...
        Def.Quad{
            InitCommand=function (self)
				self:CenterX(y,SCREEN_CENTER_Y-148):zoomto(_screen.w * 0.5 * pnSide(pn),1):horizalign(left):fadeleft(0.75):cropleft(0.15):diffuse(PlayerColor(pn));
			end;
        },
        ...
        Def.ActorFrame{
            InitCommand=function (self)
				self:x(SCREEN_CENTER_X + (pnSide(pn)*(spacing+32))):y(SCREEN_CENTER_Y-148);
			end;
            ...
            Def.BitmapText{
                ...
                InitCommand=function (self)
					self:x(-48*pnSide(pn)):zoomy(0.31):zoomx(0.3075):horizalign(pnAlign(OtherPlayer[pn])):strokecolor(BoostColor(PlayerColor(pn,0.9),1/3)):diffusealpha(0);
				end;
...
local bg = Def.ActorFrame{
    ...
    Def.Quad{
        InitCommand=function (self)
			self:zoomto(_screen.w,_screen.h):cropbottom(1/3):diffuse(BoostColor(Global.bgcolor,0.75)):diffusebottomedge(BoostColor(AlphaColor(Global.bgcolor,0.5),0.5)):fadebottom(1/3);
		end;
    },

    Def.Quad{
        InitCommand=function (self)
			self:zoomto(_screen.w,_screen.h):diffuse(BoostColor(Global.bgcolor,0.6)):cropbottom(1/25):fadetop(0.5);
		end;
    },

    LoadActor(THEME:GetPathG("","_pattern"))..{
        InitCommand=function (self)
			self:zoomto(_screen.w,_screen.h):blend(Blend.Add):diffuse(BoostColor(HighlightColor(1),0.1)):diffusebottomedge(0.1,0.1,0.1,0):fadetop(1):customtexturerect(0,0,_screen.w / 384 * 2.5 ,_screen.h / 384 * 2.5):texcoordvelocity(0,-0.075);
		end;
    },
};
